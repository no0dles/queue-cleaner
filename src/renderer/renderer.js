...
const environmentIndicatorEl = document.getElementById('environment-indicator');
...
function attachEvents() {
  if (environmentSelectEl) {
    environmentSelectEl.addEventListener('change', async () => {
      const nextEnvironment = environmentSelectEl.value || null;
...
function getCurrentEnvironmentMeta() {
  return state.environments.find((env) => env.key === state.environment) || null;
}

async function loadEnvironmentOptions() {
  try {
    const data = await window.api.getEnvironments();
    const environments = data?.environments || [];
    const defaultKey = data?.defaultKey || null;

    state.environments = environments;

    if (state.environment && environments.some((env) => env.key === state.environment)) {
      // keep existing selection if still available
    } else if (defaultKey && environments.some((env) => env.key === defaultKey)) {
      state.environment = defaultKey;
    } else if (environments.length) {
      state.environment = environments[0].key;
    } else {
    ...
    state.environment = null;
    renderEnvironmentOptions();
    setStatus(`Failed to load environments: ${error.message}`);
  }
}
...
function renderEnvironmentOptions() {
  ...
  if (!state.environments.length) {
    ...
    environmentSelectEl.disabled = true;
    updateEnvironmentIndicator();
    return;
  }
  ...
  environmentSelectEl.disabled = state.environments.length <= 1;
  if (state.environment) {
    environmentSelectEl.value = state.environment;
  }
...
function updateEnvironmentIndicator() {
  if (!environmentIndicatorEl) {
    return;
  }

  environmentIndicatorEl.textContent = '';
  environmentIndicatorEl.classList.remove('visible', 'prod');
  environmentIndicatorEl.removeAttribute('aria-label');

  const current = getCurrentEnvironmentMeta();
  if (!current) {
    return;
  }

  const label = current.isProd ? 'Production' : current.label;
  environmentIndicatorEl.textContent = label;
  environmentIndicatorEl.classList.add('visible');
  environmentIndicatorEl.classList.toggle('prod', Boolean(current.isProd));
  environmentIndicatorEl.setAttribute(
    'aria-label',
    current.isProd ? 'Production environment active' : `${current.label} environment active`
  );
}
...
function resetFilters() {
  ...
  state.searchTerm = '';
  if (vhostFilterEl) {
    vhostFilterEl.value = DEFAULT_VHOST_FILTER;
  }
  if (queueSearchEl) {
    queueSearchEl.value = '';
  }
...
async function loadQueues(options = {}) {
  ...
  const currentEnvironment = getCurrentEnvironmentMeta();
  const environmentLabel = currentEnvironment ? currentEnvironment.label : state.environment;

  if (!state.environment) {
    state.queues = [];
    queueListEl.innerHTML = '<p>No RabbitMQ environments configured.</p>';
    setStatus('No RabbitMQ environments configured.');
    toggleQueueDetails(false);
    return;
  }
  ...
  if (!silent) {
    setStatus(`Loading queues${environmentLabel ? ` for ${environmentLabel}` : ''}...`);
  ...
  try {
    const queues = await window.api.listQueues({ environment: state.environment });
    state.queues = queues;
    ...
    renderMessages();
    setStatus(
      `Loaded ${queues.length} queues with DLX${environmentLabel ? ` for ${environmentLabel}` : ''}.`
    );
  } catch (error) {
    console.error(error);
...
async function loadMessages() {
  if (!state.selectedQueue || !state.selectedDlxQueue || !state.environment) return;
  const count = Number(messageCountEl.value) || 50;
  const currentEnvironment = getCurrentEnvironmentMeta();
  const environmentLabel = currentEnvironment ? currentEnvironment.label : state.environment;

  setStatus(
    `Loading messages from ${state.selectedDlxQueue.name}${
  ...
  try {
    ...
    renderMessages();
    setStatus(
      `Loaded ${messages.length} messages from ${state.selectedDlxQueue.name}${
        environmentLabel ? ` (${environmentLabel})` : ''
      }.`
    );
  } catch (error) {
...
async function handleMoveMessage(message, button) {
  ...
  setStatus(
    `Moving message back to processing queue${environmentLabel ? ` (${environmentLabel})` : ''}...`
  );
  ...
  try {
    ...
    setStatus(
      `Message moved successfully${environmentLabel ? ` (${environmentLabel})` : ''}. Refreshing...`
    );
...
async function handleMoveAllMessages(button) {
  if (!state.selectedQueue || !state.selectedDlxQueue || !state.environment) {
    return;
  }
  ...
  const environmentLabel = currentEnvironment ? currentEnvironment.label : state.environment;
  setStatus(
    `Moving all messages back to processing queue${environmentLabel ? ` (${environmentLabel})` : ''}...`
  );
  ...
  try {
    ...
    setStatus(
      `All messages moved${environmentLabel ? ` (${environmentLabel})` : ''}. Refreshing...`
    );
    await loadQueues({ preserveSelection: true, silent: true });
    await loadMessages();
  } catch (error) {
